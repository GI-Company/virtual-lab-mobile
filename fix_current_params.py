...
# I'll replace the currentControlParams update logic in updateRepeatingRequest.
old_update = """                                if (!appliedSent && request.tag == requestId) {
                                    appliedSent = true
                                    currentControlParams = requested
                                    appliedSent = true"""

new_update = """                                if (!appliedSent && request.tag == requestId) {
                                    appliedSent = true
                                    
                                    // Keep currentControlParams from claiming rejected/unconfirmed values
                                    // We only update if it was actually applied or partially applied.
                                    // For simplicity, since we derive results below, we can build a verified copy.
                                    val safeParams = com.example.protocol.v1.ControlParameters(
                                        ...
                                        resolution = requested.resolution, // Resolution is always applied during reconfigure
...
# Wait, parameterResults are populated BELOW this line!
# I need to move the `currentControlParams` update to the bottom of the onCaptureCompleted block, right before sendResult.
